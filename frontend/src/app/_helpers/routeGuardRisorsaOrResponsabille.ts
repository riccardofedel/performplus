...
export class RouteGuardRisorsaOrResponsabile implements CanActivate {
  constructor(
  ...
  public canActivate(
    ...
    if (currentUser && currentUser.operator) {
      return (
        (currentUser.operator.valutatore || currentUser.operator.valutato) &&
        (currentUser.operator.ruolo == "RISORSA" ||
